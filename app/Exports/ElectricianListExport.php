...
class ElectricianListExport implements FromCollection
{
    ...
    public function collection()
    {
        ...
        $objNewItemHeader['architect_status'] = 'Electrician Status';
        $objNewItemHeader['account_status'] = 'Account Status';

        $objNewItemHeader['id'] = 'ERP ID';

        // $objNewItemHeader['id'] = 'Name';
        // $objNewItemHeader['type'] = 'Type';
        // $objNewItemHeader['name'] = 'Name';
        // $objNewItemHeader['phone_number'] = 'Phone Number';
        ...
        WHEN electrician.status = 0 THEN "Rejected"
        WHEN electrician.status = 1 THEN "On Boarded"
        WHEN electrician.status = 2 THEN "Entry"
        WHEN electrician.status = 3 THEN "Data Mismatch"
        WHEN electrician.status = 4 THEN "Verified by Telecaller"
        WHEN electrician.status = 5 THEN "Duplicate"
        WHEN electrician.status = 7 THEN "Not Recieved"
        WHEN electrician.status = 8 THEN "Data Pending"
        WHEN electrician.status = 9 THEN "Language Issue"
        ELSE "Undifine"
        END AS status');
        $query->selectRaw('CASE
        ...
        // $query->where('electrician.total_point','>', 50);
        // $query->whereIn('electrician.status', [0,7,8,9]);
        ...
        foreach ($query as $key => $value) {
			...
			// $LeadWonCount->whereIn('lead_sources.source_type', ['user-301','user-302']);
            ...
            // $LeadLostCount->whereIn('lead_sources.source_type', ['user-301','user-302']);
            ...
            // $LeadRunningCount->whereIn('lead_sources.source_type', ['user-301','user-302']);
            ...
            // $LeadTotalCount->whereIn('lead_sources.source_type', ['user-301','user-302']);

...
class InquiryController extends Controller
{
	...
	public function __construct()
	{
		$this->middleware(function ($request, $next) {
			$tabCanAccessBy = array(0, 1, 2, 8, 9, 202, 302, 101, 102, 102, 103, 104);
			...
			// 	$response = errorRes("you can't access this module", 401);
			// 	return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
			// }
			return $next($request);
		});
	...
	function preQuestions()
	{
		...
		$response = successRes("Pre Question");
	...
	public function getSourceTypes()
	{
		...
		$response = successRes("source types");
		$response['data'] = $sourceTypes;
	...
	{
		$response = errorRes("Inquiry Module Is Closed Please Contact To Admin", 401);
		return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
		$InquiryQuestions = InquiryQuestion::whereIn('id', array(8, 9, 10))->orderBy('status', 'asc')->orderBy('sequence', 'asc')->get();
	...
	function pendingRequestAjax(Request $request)
	{
		...
		if ($isSalePerson == 1 || $isChannelPartner != 0) {
			...
			foreach ($data as $key => $value) {
				...
				$data[$key]['created_by'] .= "<p class='border-box mb-0 " . $primeArchitectClass . "'><i  data-bs-toggle='tooltip' title='Architect Name' class='bx bxs-user bx-sm extrasmallfont'></i><span data-bs-toggle='tooltip' title='" . $data[$key]['architect_name'] . "' > " . displayStringLenth($data[$key]['architect_name'], 25) . "</span> <br> <i class='bx bx-phone bx-sm extrasmallfont' data-bs-toggle='tooltip' title='Architect Mobile No.' ></i> " . $data[$key]['architect_phone_number'] . "</p>";

				$data[$key]['created_by'] .= "<p class='border-box mb-0 " . $primeEleactricanClass . "'><i  data-bs-toggle='tooltip' title='Electrician Name' class='bx bxs-user bx-sm extrasmallfont'></i> <span  data-bs-toggle='tooltip' title='" . $data[$key]['electrician_name'] . "' >" . displayStringLenth($data[$key]['electrician_name'], 25) . "</span> <br> <i class='bx bx-phone bx-sm extrasmallfont' data-bs-toggle='tooltip' title='Electrician Mobile No.' ></i> " . $data[$key]['electrician_phone_number'] . "</p>";
				...
				if ($data[$key]['billing_invoice'] == "") {
					...
					$data[$key]['billing_invoice'] = "<a class='btn btn-sm btn-success btn-quotation' onclick='openBillingInvoiceModal(\"" . $data[$key]['billing_invoice'] . "\")'  href='javascript:void(0)' data-bs-toggle='tooltip' title='Billing Invoice' >Billing Invoice</a>";
				}
	...
	function ajax(Request $request)
	{
		...
		$advanceFilterType = $request->advance_filter_type ? $request->advance_filter_type : '[]';
		$advanceFilterText = $request->advance_filter_text ? $request->advance_filter_text : '[]';
		$advanceFilterText = $request->advan;
		...
		if ($isAdminOrCompanyAdmin == 1) {
			...
			$childSalePersonsIds = getChildSalePersonsIds(Auth::user()->id);
			$query->whereIn('assigned_to', $childSalePersonsIds);
		...
		if ($closingType != "0" && $request->status == 8) {
			...
			if ($closingType == "1") {
				...
				$monthEndDay = date('Y-m-t H:i:s', strtotime($currentEndDatetime . " +1 month"));
		...
		if ($isArchitect == 1) {
			...
			// if ($request->status != 0) {
				...
				// $statusArray = isset($inquiryStatus[201]['for_architect_ids']) ? $inquiryStatus[201]['for_architect_ids'] : array(0);
			...
			// 	$statusArray = isset($inquiryStatus[201]['for_electrician_ids']) ? $inquiryStatus[201]['for_electrician_ids'] : array(0);
			...
			$statusArray = isset($inquiryStatus[201]['for_channel_partner_ids']) ? $inquiryStatus[201]['for_channel_partner_ids'] : array(0);
		...
		foreach ($data as $key => $value) {
			...
			$data[$key]['status_lable'] = $InquiryStatus[$value->status]['name'];
		}
		$data = json_decode(json_encode($data), true);

		$response = successRes("Inquiry");
		$response['data'] = $data;
		return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
		...
		// 			<p class="border-box mb-0"><i  data-bs-toggle="tooltip" title="Status" class="bx bx-disc bx-sm extrasmallfont"></i><i  data-bs-toggle="tooltip" title="Status Update Date & Time : ' . $valueAnserDateTime . '" class="bx bx-calendar bx-sm extrasmallfont"></i>';
		...
		// 					$data[$key]['status'] .= '<button type="button" class="btn btn-info waves-effect waves-light btn-sm m-1 btn-edit-detail" onclick="editBillingInvoice(' . $data[$key]['inquiry_id'] . ',' . $claimed24HoursIn . ')"> EDIT</button>';
		// 				}
		...
		// 			$data[$key]['name'] = '<p class="border-box font-size-14 mb-0 text-dark d-flex justify-content-between align-items-center"><span><span class="lable-inquiry-id" id="inquiry-id-' . $data[$key]['id'] . '" >#' . $data[$key]['id'] . '</span><span id="inquiry-name-' . $data[$key]['id'] . '" data-bs-toggle="tooltip" title="' . $value['first_name'] . " " . $value['last_name'] . '" > ' . displayStringLenth($value['first_name'] . " " . $value['last_name'], 25) . ' </span></span>';
		...
		// 			$data[$key]['architect'] = "<p class='border-box mb-0 " . $primeArchitectClass . "'><i  data-bs-toggle='tooltip' title='Architect Name' class='bx bxs-user bx-sm extrasmallfont'></i><span data-bs-toggle='tooltip' title='" . $data[$key]['architect_name'] . "' > " . displayStringLenth($data[$key]['architect_name'], 25) . "</span> <i class='bx bx-phone bx-sm extrasmallfont' data-bs-toggle='tooltip' title='Architect Mobile No.' ></i> " . $data[$key]['architect_phone_number'] . "</p>";
		...
		// 				$data[$key]['electrician'] = "<p class='border-box mb-0 " . $primeEleactricanClass . "'><i  data-bs-toggle='tooltip' title='Electrician Name' class='bx bxs-user bx-sm extrasmallfont'></i> <span  data-bs-toggle='tooltip' title='" . $data[$key]['electrician_name'] . "' >" . displayStringLenth($data[$key]['electrician_name'], 25) . "</span> <i class='bx bx-phone bx-sm extrasmallfont' data-bs-toggle='tooltip' title='Electrician Mobile No.' ></i> " . $data[$key]['electrician_phone_number'] . "</p>";
		...
		// 				$data[$key]['view_follow_up_type'] .= "<div class='border-box mb-0 " . $folloupDateTimeClass . "'><i  data-bs-toggle='tooltip' title='Follow up Type' class='bx bx-book-open bx-sm extrasmallfont'></i> " . $data[$key]['follow_up_type'] . " ";
		...
		// 				$data[$key]['billing'] .= '<button type="button" class="btn btn-info waves-effect waves-light btn-sm m-1 btn-edit-detail" onclick="editBillingInvoice(' . $data[$key]['inquiry_id'] . ',' . $claimed24HoursIn . ')"> EDIT</button>';
		// 			}
		...
		// 				$data[$key]['status'] = '<p class="border-box mb-0"><i  data-bs-toggle="tooltip" title="Status" class="bx bx-disc bx-sm extrasmallfont"></i><i  data-bs-toggle="tooltip" title="Status Update Date & Time : ' . $valueAnserDateTime . '" class="bx bx-calendar bx-sm extrasmallfont"></i>';
			...
			if ($request->new_inquiry_id == 0) {
				...
				if ($isArchitect == 1) {
					...
					if (isset($request->inquiry_source_type_1) && $request->inquiry_source_type_1 != "") {
						...
						$source_type_1 = isset($request->inquiry_source_type_1) ? $request->inquiry_source_type_1 : '';
					...
					if (isset($request->inquiry_source_type_2) && $request->inquiry_source_type_2 != "") {
						...
						$source_type_2 = isset($request->inquiry_source_type_2) ? $request->inquiry_source_type_2 : '';
					...
					if (isset($request->inquiry_source_type_3) && $request->inquiry_source_type_3 != "") {
						...
						$source_type_3 =  isset($request->inquiry_source_type_3) ? $request->inquiry_source_type_3 : '';
					...
					if (isset($request->inquiry_source_type_4) && $request->inquiry_source_type_4 != "") {
						...
						$source_type_4 =  isset($request->inquiry_source_type_4) ? $request->inquiry_source_type_4 : '';
				...
				if ($Inquiry) {
					...
					if ($Inquiry->source_type == "user-202" || $Inquiry->source_type == "user-302") {
				...
				if ($Inquiry && ($isAdminOrCompanyAdmin == 1 || ($isThirdPartyUser == 1 && $Inquiry->source_type_value == Auth::user()->id) || ($isSalePerson == 1 && in_array($Inquiry->assigned_to, $childSalePersonsIds)) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id))) {
	...
	public function detail(Request $request)
	{
		...
		if (($isAdminOrCompanyAdmin == 1) || ($isSaleArchitect == 1) || ($isSaleElectrician == 1) || ($isThirdPartyUser == 1) || ($isSalePerson == 1 && in_array($Inquiry->assigned_to, $childSalePersonsIds)) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id)) {

			$response = successRes("Successfully get Inquiry Detail");
			...
			if ($data['ui_type'] == "inquiry_update") {
				...
				$response = successRes("Inquiry Detail");
				$response['data'] = $data;
				return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
			} elseif ($data['ui_type'] == "inquiry_files") {
				$response['files'] = InquiryQuestionAnswer::where('inquiry_id', $request->inquiry_id)->where('inquiry_question_id', 1)->where('question_type', 2)->where('answer', '!=', '')->where('inquiry_id', $request->inquiry_id)->orderBy('id', 'desc')->get();
			} else if ($data['ui_type'] == "inquiry_log") {

				$response['log'] = InquiryLog::select('inquiry_log.id', 'inquiry_log.name', 'inquiry_log.description', 'inquiry_log.created_at', 'inquiry_log.user_id', 'users.first_name', 'users.last_name')->leftJoin('users', 'inquiry_log.user_id', '=', 'users.id')->where('inquiry_log.inquiry_id', $request->inquiry_id)->orderBy('inquiry_log.id', 'desc')->get();
				...
				$Inquiry['status_label'] = getInquiryStatus()[$Inquiry['status']]['name'];
				$Inquiry['stage_of_site_label'] = $Inquiry['stage_of_site'];
				if ($source_type_pieces[0] == "user") {
				...
				$User = User::select('users.id', 'users.first_name', 'users.last_name', DB::raw("CONCAT(users.first_name,' ',users.last_name) AS full_name"));
				$User->where('id', $Inquiry['user_id']);
				$User->limit(1);
				$User = $User->first();
				if ($User) {

					$response['detail']['created_by'] = array();
					$response['detail']['created_by']['id'] = $User->id;
					$response['detail']['created_by']['text'] = $User->full_name;
				}

				$User = User::select('users.id', 'users.first_name', 'users.last_name','users.phone_number', DB::raw("CONCAT(users.first_name,' ',users.last_name) AS full_name"));
				...
				if ($User) {
					...
					$response['detail']['architect']['phone_number'] = $User->phone_number;
				} else {
					$response['detail']['architect'] = (object) array();
				}
				...
				if ($User) {
					...
					$response['detail']['electrician']['phone_number'] = $User->phone_number;
				} else {
					$response['detail']['electrician'] = (object) array();
				}
	...
	public function searchUser(Request $request)
	{
		...
		if (isset($request->user_id) && $request->user_id != "") {
			...
			if ($isChannelPartner != 0 && ($request->source_type == 201 || $request->source_type == 202)) {
				...
				$UserResponse = array();
				$q = $request->q;
				$User = User::select('users.id', 'users.first_name', 'users.last_name', DB::raw("CONCAT(users.first_name,' ',users.last_name) AS full_name"), 'users.phone_number');
				$User->where('users.status', 1);
		...
		$response = successRes("Search User");
		...
		if (isset($request->user_id) && $request->user_id != "") {
			...
			if (count($User) > 0) {
				foreach ($User as $User_key => $User_value) {
					$UserResponse[$User_key]['id'] = $User_value['id'];
					$UserResponse[$User_key]['text'] = $User_value['full_name'];
					$UserResponse[$User_key]['phone_number'] = $User_value['phone_number'];
				}
			}
		}

		$response = successRes("Search architect");
		$response['data'] = $UserResponse;
		return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
	}
	...
	public function searchElectrician(Request $request)
	{
		...
		$response = successRes("Search electrician");
		$response['data'] = $UserResponse;
		return response()->json($response, $response['status_code'])->header('Content-Type', 'application/json');
	}
	...
	public function assignedUser(Request $request)
	{
		...
		if ($inquiry) {
			...
			$User = $User->get();
			if (count($User) > 0) {
				$response = successRes("get assigned user");
				$response['data'] = $User;
			} else {
				$response = errorRes("");
			}
	...
	public function saveBilling(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			if ($Inquiry) {
				$isSalePerson = isSalePerson();
				$isAdminOrCompanyAdmin = isAdminOrCompanyAdmin();
				if ($isAdminOrCompanyAdmin == 1 || $isThirdPartyUser == 1 || $isSalePerson == 1 || ($isChannelPartner != 0)) {
		...
		if ($validator->fails()) {
			...
			if ($isAdminOrCompanyAdmin == 1 || $isThirdPartyUser == 1 || $isSalePerson == 1 || $isChannelPartner != 0) {
				...
				if ($Inquiry) {
					...
					if ($isAdminOrCompanyAdmin == 1 || ($isThirdPartyUser == 1 && $Inquiry->source_type_value == Auth::user()->id) || ($isSalePerson == 1 && in_array($Inquiry->assigned_to, $childSalePersonsIds)) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id)) {
	...
	public function saveFollowUpDateTime(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			$Inquiry = Inquiry::select('id', 'assigned_to', 'follow_up_date_time')->find($request->inquiry_id);
			if ($Inquiry) {
				...
				if ($isAdminOrCompanyAdmin == 1 || ($isThirdPartyUser == 1) || ($isSalePerson == 1 && (in_array($assigned_to, $childSalePersonsIds))) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id)) {
	...
	public function saveClosingDateTime(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			$Inquiry = Inquiry::select('id', 'assigned_to', 'closing_date_time')->find($request->inquiry_id);
			if ($Inquiry) {
				...
				if ($isAdminOrCompanyAdmin == 1 || ($isThirdPartyUser == 1 && $Inquiry->source_type_value == Auth::user()->id) || ($isSalePerson == 1 && (in_array($assigned_to, $childSalePersonsIds))) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id)) {
	...
	public function saveInquiryAnswer(Request $request)
	{
		...
		if ($Inquiry) {
			...
			if ($isAdminOrCompanyAdmin == 1 || ($isSalePerson == 1 && (in_array($Inquiry->assigned_to, $childSalePersonsIds))) || ($isChannelPartner != 0 && $Inquiry->source_type_value == Auth::user()->id)) {
				...
				if (count($inquiryQuestion) > 0) {
					foreach ($inquiryQuestion as $iQK => $iQV) {
						...
						if ($iQV->is_depend_on_answer == 1) {
							...
							// Validation If question is  depended on other question
	...
	public function updateSeen(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			$response['data'] = $InquiryUpdateSeen;
		}
	...
	function searchMentionUsers(Request $request)
	{
		...
		$User->whereIn('users.type', array(0, 1, 2, 101, 102, 103, 104));
		$User->where('users.status', 1);

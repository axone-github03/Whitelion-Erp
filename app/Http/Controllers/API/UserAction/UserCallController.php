...
class UserCallController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (isset($request->call_move_to_close) && $request->call_move_to_close == "1" || $request->call_type_id == 2) {
                ...
                $UserCall->outcome_type = $request->call_outcome;
    ...
    function searchContact(Request $request)
    {
        ...
        if(isset($request->user_id) && $request->user_id != 0) {
            ...
            $response = errorRes("User Not Valid");
        }
    ...
    function detail(Request $request)
    {
        ...
        if ($LeadCall) {
            ...
            $LeadCall['schedule']['id'] = date('h:i A', strtotime($LeadCall['call_schedule']));
            $LeadCall['schedule']['text'] = date('h:i A', strtotime($LeadCall['call_schedule']));
        
            $LeadCall['reminder_data']['id'] = getReminderTimeSlot()[$LeadCall['reminder_id']]['name'];
            $LeadCall['reminder_data']['text'] = getReminderTimeSlot()[$LeadCall['reminder_id']]['name'];

...
class CRMUserOrderController extends Controller
{
	...
	public function ajax(Request $request)
	{
		...
		foreach ($data as $key => $value) {
			...
			if ($value['gift_product_order_query_id'] != 0) {
				...
				if ($GiftProductOrderQuery->status == 0) {

					$redirectURL = route('crm.user.order.query.detail');
					$uiAction .= '<li class="list-inline-item px-2">';
					$uiAction .= '<a href="' . $redirectURL . '?id=' . $value['gift_product_order_query_id'] . '" title="Raise Query" class="btn btn-warning waves-effect waves-light btn-sm raise-query-btn">Ticket #' . $value['gift_product_order_query_id'] . '</a>';
					$uiAction .= '</li>';
				} else {

					$redirectURL = route('crm.user.order.query.detail');
					$uiAction .= '<li class="list-inline-item px-2">';
					$uiAction .= '<a href="' . $redirectURL . '?id=' . $value['gift_product_order_query_id'] . '" title="Raise Query" class="btn btn-success waves-effect waves-light btn-sm raise-query-btn">Ticket #' . $value['gift_product_order_query_id'] . '</a>';
					$uiAction .= '</li>';

					$uiAction .= '<li class="list-inline-item px-2">';
					$uiAction .= '<a onclick="RaiseQuery(\'' . $value['id'] . '\')" href="javascript: void(0);" title="Raise Query" class="btn btn-primary waves-effect waves-light btn-sm raise-query-btn">Raise Query <i class="bx bx-support"></i></a>';
					$uiAction .= '</li>';
				}
			} else {
			...
			foreach ($cartIterms as $key => $value) {
				...
				if (!in_array($value['id'], array_keys($idWithKey))) {
					...
					if ($GiftProduct->status == 1) {
						...
						$finalCartItems[$cfinalCartItems]['has_cashback'] = $GiftProduct->has_cashback;
						$finalCartItems[$cfinalCartItems]['cashback'] = $GiftProduct->cashback;
						$finalCartItems[$cfinalCartItems]['item_value'] = $GiftProduct->price;
					}
				} else {
				...
				if ($userTotalPoint >= ($orderDetail['total_product_pv'] + $totalCashPV)) {
					...
					if (($orderDetail['total_cashback_value'] + $totalCash) > 0) {
						...
						if (!$hasAnyOrder) {
							...
							if ($totalCashPV > 0 && ($totalCashPV + $orderDetail['total_product_pv']) <= 100) {
					...
					foreach ($orderDetail['items'] as $key => $value) {
						...
						$GiftProductOrderItem->item_value = $value['item_value'];
						$GiftProductOrderItem->total_item_value = $value['item_value'] * $value['qty'];
					...
					$params['point_used'] =  $GiftProductOrder->total_point_value;
					$params['available_point'] =  (int)$userTotalPoint - (int)$GiftProductOrder->total_point_value;
					$params['total_cash'] = $GiftProductOrder->total_cash;
					$params['order_date'] =  convertDateTime($GiftProductOrder->created_at);
					$UserDetail =  User::select('email', 'first_name', 'last_name','phone_number')->find(Auth::user()->id);
					$params['user_name'] =  $UserDetail->first_name . ' ' . $UserDetail->last_name;
					$params['to_email'] = $UserDetail->email;
					$params['user_mobile'] = $UserDetail->phone_number;
					// $params['to_name'] = $configrationForNotify['to_name'];
					// $params['query'] = $GiftProductOrderQuery;
					$params['subject'] = "Reward Request Received";
					...
					//TEMPLATE 10
					Mail::send('emails.architect_request_to_claim_gift', ['params' => $params], function ($m) use ($params) { // SEND MAIL

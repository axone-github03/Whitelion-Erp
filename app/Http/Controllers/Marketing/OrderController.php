...
class OrderController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->middleware(function ($request, $next) {
			...
			$tabCanAccessBy = array(0, 1, 2, 6, 13);
	...
	{
		$data = array();
		$data['title'] = "Marketing Request";
		return view('marketing/orders/index', compact('data'));
	}
	...
	function ajax(Request $request)
	{
		...
		$recordsTotal = MarketingOrder::query();
		...
		if ($isAdminOrCompanyAdmin == 1) {
			...
			$recordsTotal->orwhere(function ($recordsTotal) {
				$recordsTotal->Where('marketing_orders.is_self',1);
				$recordsTotal->Where('marketing_orders.user_id',Auth::user()->id);
			});
	...
	public function detail(Request $request)
	{
		...
		if ($Order) {
			...
			// 	$OrderItem->select('marketing_order_items.id', 'marketing_order_items.qty', 'marketing_order_items.total_mrp', 'marketing_order_items.pending_qty', 'marketing_order_items.marketing_product_inventory_id', 'marketing_order_items.width','marketing_order_items.height','marketing_order_items.box_image','marketing_order_items.sample_image', 'marketing_product_inventory.thumb as product_image', 'product_code.name as product_code_name', 'marketing_product_inventory.quantity as product_stock','marketing_product_inventory.is_custome', 'marketing_order_items.gst_percentage', 'marketing_order_items.gst_tax', 'marketing_order_items.total_gst_tax');
			// } else {
			// 	$OrderItem->select('marketing_order_items.id', 'marketing_order_items.qty', 'marketing_order_items.total_mrp', 'marketing_order_items.pending_qty', 'marketing_order_items.marketing_product_inventory_id','marketing_order_items.width','marketing_order_items.height','marketing_order_items.box_image','marketing_order_items.sample_image', 'marketing_product_inventory.thumb as product_image', 'product_code.name as product_code_name', 'marketing_order_items.gst_percentage', 'marketing_order_items.gst_tax', 'marketing_order_items.total_gst_tax');
			// }
			if ($isAdminOrCompanyAdmin == 1) {
				$OrderItem->select(
	...
	public function add()
	{
		...
		$data['title'] = "Add Marketing Request ";
		return view('marketing/orders/add', compact('data'));
	}
	...
	public function searchChannelPartner(Request $request)
	{
		if (isset($request->channel_partner_type) && $request->channel_partner_type != 0) {
			...
			$ChannelPartner = User::select('id', DB::raw('CONCAT(users.first_name," ", users.last_name) AS text'))->where('id', Auth::user()->id)->get();
		}
	...
	function searchProduct(Request $request)
	{
		...
		$DataMaster = MarketingProductInventory::select('marketing_product_inventory.id', DB::raw('CONCAT(product_code.name," (",marketing_product_inventory.description,")" )  as text'));
		...
		$DataMaster->where('marketing_product_inventory.is_custome','!=', 1);
		...
		$DataMaster->where(function ($query) use ($searchValue) {
			$query->where('product_code.name', 'like', '%' . $searchValue . '%');
			$query->orWhere('marketing_product_inventory.description', 'like', '%' . $searchValue . '%');
		});

		// if (count($searchValuePieces) > 1) {

		// 	$DataMaster->where(function ($query1) use ($searchValuePieces) {
		// 		$query1->where(function ($query) use ($searchValuePieces) {


		// 			// $query->where('product_group.name', 'like', $searchValuePieces[0] . "%");
		// 			// $query->Where('product_code.name', 'like', $searchValuePieces[1] . "%");

		// 			$query->Where('product_code.name', 'like', $searchValuePieces[1] . "%");
		// 		});
		// 		$query1->orWhere('marketing_product_inventory.description', 'like', implode(" ", $searchValuePieces) . "%");
		// 	});
		// } else {

		// 	$DataMaster->where(function ($query) use ($searchValue) {
		// 		// $query->where('product_group.name', 'like', $searchValue . "%");
		// 		// $query->orWhere('marketing_product_inventory.description', 'like', $searchValue . "%");


		// 		$query->where('marketing_product_inventory.description', 'like', $searchValue . "%");
		// 	});
		// }
	...
	{
		$MarketingProductInventory = MarketingProductInventory::with(array('product_group' => function ($query) {
			$query->select('id', 'name');
		...
		if ($request->expectsJson()) {
			...
			foreach ($inputJSON['order_items'] as $key => $value) {
				...
				if ($value['is_custome'] == 1) {
					$orderItems[$key]['width'] = (int)$value['width'];
					$orderItems[$key]['height'] = (int)$value['height'];
		...
		foreach ($orderDetail['items'] as $key => $value) {
			...
			if($value['is_custom'] == 1)
			{
				if ($value['box_image'] != '') {
					...
					$fileObject1 = base64_decode(explode(',', $value['box_image'])[1]);
				...
				if ($value['sample_image'] != '' && $value['sample_image'] != "" &&   $value['sample_image'] != null) {
					...
					$fileObject1 = base64_decode(explode(',', $value['sample_image'])[1]);
					$extension = '.png';
					$fileName1 = uniqid() . '_' . $value['id'] . $extension;
					$destinationPath = public_path($folderPathofFile);

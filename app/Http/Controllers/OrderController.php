...
class OrderController extends Controller
{
	...
	public function __construct()
	{
		...
		$this->middleware(function ($request, $next) {
			...
			$tabCanAccessBy = array(0, 1, 2, 3, 101, 102, 103, 104, 105, 13);
	...
	function ajax(Request $request)
	{
		...
		if ($isSalePerson == 1) {
			...
			// foreach ($childSalePersonsIds as $key => $value) {
			// 	if ($key == 0) {
			// 		$recordsTotal->whereRaw('FIND_IN_SET("' . $value . '",orders.sale_persons)>0');
			// 	} else {
			// 		$recordsTotal->orWhereRaw('FIND_IN_SET("' . $value . '",orders.sale_persons)>0');
			// 	}
		...
		foreach ($data as $key => $value) {
			...
			if ($value['invoice'] != "") {
				...
				$routeInvoice = route('orders.invoice.list') . "?order_id=" . $value['id'];
				if(isCreUser() == 0) {
					$uiAction .= '<li class="list-inline-item px-2">';
	...
	public function detail(Request $request)
	{
		...
		if ($Order) {
			...
			$response['data'] = $Order;
			$response['data']['created_at_timestamp'] = strtotime($Order['created_at']);
		} else {
			$response = errorRes("Invalid order id");
		}
		return response()->json($response)->header('Content-Type', 'application/json');
	...
	public function createdSave(Request $request)
	{
		...
		if (Auth::user()->type == 0) {
			...
			$Order = Order::find($request->order_date_time_id);
	...
	function export(Request $request)
	{
		...
		if ($request->export_type == 0 || $request->export_type == 1) {
			...
			$Order = Order::query();
			$Order->select('orders.id', 'channel_partner.firm_name', 'orders.created_at','users.first_name', 'users.last_name');
			$Order->leftJoin('channel_partner', 'channel_partner.user_id', '=', 'orders.channel_partner_user_id');
			$Order->leftJoin('users', 'users.id', '=', 'orders.user_id');
			$Order->orderBy('orders.id', 'desc');
			$Order->whereDate('orders.created_at', '>=', $startDate);
			$Order->whereDate('orders.created_at', '<=', $endDate);
			...
			$Order->whereDate('orders.created_at', '>=', $startDate);
			$Order->whereDate('orders.created_at', '<=', $endDate);
			$Order->whereIn('orders.status', array(4));
			if (isset($request->channel_partner_user_id) && is_array($request->channel_partner_user_id)) {

...
class UsersPurchasePersonController extends Controller
{
    ...
    {
        $data = array();
        $data['title'] = "Purchase -  Users";
        $data['country_list'] = CountryList::get();
        $data['type'] = 2;
        return view('users/purchase', compact('data'));
    }
    ...
    public function ajax(Request $request)
    {
        ...
        $selectColumns = array(
            ...
            'users.first_name',
            'users.email',
            'purchase_person.reporting_manager_id',
            'users.status',
            'users.last_name',
            'users.type',
            'users.status',
            ...
            'purchase_hierarchy.code as purchase_hierarchy_code',
            'users.last_login_date_time',

        );

        $recordsTotal = DB::table('users')->where('type', 10)->count();
        $recordsFiltered = $recordsTotal; // when there is no search parameter then total number rows = total number filtered rows.
        $query = DB::table('users');
        $query->leftJoin('companies', 'companies.id', '=', 'users.company_id');
        $query->leftJoin('purchase_person', 'purchase_person.id', '=', 'users.reference_id');
        ...
        foreach ($data as $key => $value) {
            ...
            $data[$key]['name'] = '<h5 class="font-size-14 mb-1"><a href="javascript: void(0);" class="text-dark">' . highlightString($value['first_name'] . " " . $value['last_name'],$search_value) . '</a></h5>
             <p class="text-muted mb-0">' . highlightString(getUserTypeName($value['type']),$search_value) . '</p><p class="text-muted mb-0">' . highlightString($value['purchase_hierarchy_code'],$search_value) . '</p>';

            if ($value['reporting_manager_id'] != 0) {

                $reportingManager = User::select('first_name', 'last_name', 'email', 'purchase_hierarchy.code')->leftJoin('purchase_person', 'purchase_person.user_id', '=', 'users.id')->leftJoin('purchase_hierarchy', 'purchase_hierarchy.id', '=', 'purchase_person.type')->find($value['reporting_manager_id']);
                if ($reportingManager) {
    ...
    function export()
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $reportingManager = User::select('first_name', 'last_name', 'purchase_hierarchy.code')->leftJoin('purchase_person', 'purchase_person.user_id', '=', 'users.id')->leftJoin('purchase_hierarchy', 'purchase_hierarchy.id', '=', 'purchase_person.type')->find($value->reporting_manager_id);

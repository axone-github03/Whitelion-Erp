...
class CreateTriggerNewUpdateQuotationTableOnDelete extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::unprepared('
        DROP TRIGGER IF EXISTS `update_quotation_table_on_delete`;CREATE DEFINER=`root`@`localhost` TRIGGER `update_quotation_table_on_delete` AFTER DELETE ON `wltrn_quot_itemdetails` FOR EACH ROW BEGIN
...
-- Calculate Billing Amount For New Row
  SELECT
  IFNULL(SUM((wltrn_quot_itemdetails.net_amount-(wltrn_quot_itemdetails.net_amount*wlmst_item_prices.channel_partners_discount/100))+((wltrn_quot_itemdetails.net_amount-(wltrn_quot_itemdetails.net_amount*wlmst_item_prices.channel_partners_discount/100))*18/100)), 0)
  INTO
  ...
  WHERE
    wltrn_quot_itemdetails.quot_id = OLD.quot_id
    AND 
...
END
    ...
    {
        DB::unprepared('DROP TRIGGER `update_quotation_table_on_delete`');
    }
}

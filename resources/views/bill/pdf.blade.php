...
@endif
                                                       <br>
                                                        {{$data['d_pincode']}}
                                                        <br>
  {{getCityName($data['d_city_id'])}}, {{getStateName($data['d_state_id'])}}, {{getCountryName($data['d_country_id'])}} </td>
    </tr>
  ...
  <table width="100%">
   ...
   @foreach($data['items'] as $key=>$value)
    ...
    <tr>
        ...
        <td align="right">{{$value['product_brand_name']}} </td>
        <td align="right">{{$value['product_code_name']}}</td>
        <td align="right">{{priceLable($value['mrp'])}}</td>
        <td align="right">{{$value['qty']}}</td>
        <td align="right">{{$value['discount_percentage']}}%</td>
        <td align="right">{{priceLable($value['mrp_minus_disocunt'])}}</td>
      </tr>

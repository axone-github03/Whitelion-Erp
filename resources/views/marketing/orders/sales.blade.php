            ...
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    ...
                    <div class="page-title-right">
                        ...
                        @if(isCreUser() == 1)
                            <a href="{{ route('marketing.order.add') }}" class="btn btn-primary"><i class="bx bx-plus"></i>Add Request</a>
                        @endif
...
<div class="modal fade" id="modalOrder" data-bs-backdrop="static" data-bs-keyboard="true" tabindex="-1" role="dialog" aria-labelledby="modalOrderLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
        <div class="modal-content">
            ...
            <form id="formInvoice" action="{{route('marketing.orders.sales.invoice.save')}}" method="POST" class="needs-validation" novalidate>
                @csrf
                                        ...
                                        <table class="table align-middle table-nowrap mb-0">
                                            <thead class="table-light">
                                                <tr>
                                                    ...
                                                    <th>Order QTY</th>
                                                    <th>Pending QTY</th>
                                                    <th id="thApprove">Approve QTY</th>
                                                    {{-- <th id="thApprove">Width</th>
                                                    <th id="thApprove">Height</th> --}}
                                                    <th>Box Image</th>
                                                    <th>Sample Image</th>
                                                </tr>
                                            </thead>
                                         ...
                                         <div class="col-md-4">
                                            <div class="mb-3">
                                                ...
                                                <input type="file" class="form-control" id="invoice_file" name="invoice_file"
                                                    placeholder="Invoice PDF" value="" required  accept="@php
                ...
                <div class="modal-footer" id="modalFooter">
                    ...
                    <button onclick="CancelOrder()" type="button" class="btn btn-danger">Reject</button>
                    <button id="submitInvoice" type="submit" class="btn btn-primary">Approve</button>
                </div>
            </form>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    var ajaxURL = "{{route('marketing.orders.sales.ajax')}}";
    var ajaxOrderDetail = "{{route('marketing.orders.sales.detail')}}";
        ...
        $.ajax({
            ...
            success: function(resultData) {
                if (resultData['status'] == 1) {
                    ...
                    lastViewStatus = resultData['data']['status'];
                    lastViewId = resultData['data']['id'];
            ...
            success: function(resultData) {
                ...
                if (resultData['status'] == 1) {
                    ...
                    for (var i = 0; i < resultData['order']['items'].length; i++) {
                        ...
                        htmlTR += "<td>";
                        // htmlTR += "" + resultData['order']['items'][i]['info']['product_group_name'] + "<br>";
                        ...
                        if (lastViewStatus == 0) {
                            ...
                            htmlTR += "<td>";
                            htmlTR += "" + resultData['order']['items'][i]['info']['orignal_qty'] - resultData['order']['items'][i]['info']['pending_qty'] + "";
                            htmlTR += "</td>";

                        }

                        if(resultData['order']['items'][i]['is_custom'] == 1)
                        {
                            // htmlTR += "<td>";
                            // htmlTR += "" + resultData['order']['items'][i]['width'] + "";
                            // htmlTR += "</td>";
                            // htmlTR += "<td>";
                            // htmlTR += "" + resultData['order']['items'][i]['height'] + "";
                            // htmlTR += "</td>";
                            htmlTR += "<td>";
                            htmlTR += "<a href="+getSpaceFilePath(resultData['order']['items'][i]['box_image'])+" target='_blank'><img src="+getSpaceFilePath(resultData['order']['items'][i]['box_image'])+" height="+25+"></a>";
                            htmlTR += "</br><span style='font-size: 10px;'>width : "+resultData['order']['items'][i]['width']+"</span>";
                            htmlTR += "</br><span style='font-size: 10px;'>height : "+resultData['order']['items'][i]['height']+"</span>";
                            htmlTR += "</td>";
                            htmlTR += "<td>";
                            htmlTR += "<a href="+getSpaceFilePath(resultData['order']['items'][i]['sample_image'])+" target='_blank'><img src="+getSpaceFilePath(resultData['order']['items'][i]['sample_image'])+" height="+25+"></a>";
                            htmlTR += "</td>";
                        }
                        else{
                            // htmlTR += "<td>";
                    ...
                    if (lastViewStatus == 0) {
                        $("#modalFooter").show();
                        $(".input_order_item_id").prop("disabled", false);
                        $("#thApprove").html("Approve QTY");
                    } else {
                        $("#thApprove").html("Approved QTY");
                        $(".input_order_item_id").prop("disabled", true);
                        $("#modalFooter").hide();
                    }
